...
from odemis.gui.win.acquisition import OverviewAcquisitionDialog, FeatureAcquisitionDialog
...
class FeatureStreamAcquisitionController(object):
    """ controller to handle acquisition of multiple features for meteor
    """
    ...
    def open_acquisition_dialog(self):
        ...
        # create the dialog
        try:
            acq_dialog = FeatureAcquisitionDialog(
                self._tab.main_frame, self._tab_data_model)
            parent_size = [v * 0.77 for v in self._tab.main_frame.GetSize()]
